...
const returnBtnBottomContainer = document.querySelector(
  ".return-btn-bottom-container"
);
...
function buttonClick(e) {
  if (e.target.closest(".search-all-btn")) {
    searchAllRecipes(searchInput);
    displaySearchMessage(searchInput.value);
  ...
  if (
    e.target.closest(".card-icon-favorite") ||
    e.target.closest(".recipe-icon-favorite")
  ) {
  ...
  if (
    e.target.closest(".card-icon-cook") ||
    e.target.closest(".recipe-icon-cook")
  ) {
  ...
  if (e.target.classList.contains("filter-searched")) {
    removeRecipes();
    filterByTag(e, searched);
    removeReturnBtn();
    displayReturnBtn();
  }
}
...
function checkSelected(recipe, type) {
  return (
    user && user[type].find(favorite => favorite.id === recipe.id) && "selected"
  );
}
...
function getIngredientCost(ingredient, ingredientName) {
  return (
    (ingredientName.estimatedCostInCents * ingredient.quantity.amount) /
    100
  ).toFixed(2);
}
...
function displayReturnBtn() {
  returnBtnContainer.insertAdjacentHTML(
    "afterbegin",
    `
  <button type="button" name="button" class="return-btn"><ion-icon name="close-outline" class="return-btn"></ion-icon></button>
  `
  );
  returnBtnBottomContainer.insertAdjacentHTML(
    "afterbegin",
    `
  <button type="button" name="button" class="return-btn"><ion-icon name="close-outline" class="return-btn"></ion-icon></button>
  `
...
function searchAllRecipes(searchInput) {
  ...
  let recipes = [];
  let ingredients = findIngredients(input);
  ...
  filterUniqueRecipes(recipes);
}

function findIngredients(input) {
  let ingredients = [];
  ingredientsData.forEach(
    ingredient =>
      ingredient.name.includes(input) && ingredients.push(ingredient.id)
  );
  return ingredients;
}

function filterUniqueRecipes(allRecipes) {
  let uniqueRecipes = new Set(allRecipes);
  searched = [...uniqueRecipes];
  removeRecipes();
  displayFilteredRecipe(searched);
  removeTags();
  getTags(searched, "filter-searched");
}

function displayFilteredRecipe(recipes) {
  recipes.forEach(recipe => displayRecipe(recipe));
  removeReturnBtn();
  displayReturnBtn();
}

function displaySearchMessage(input) {
  input && (welcomeMessage.innerHTML = `Searched by: ${input}`);
}

function findUser() {
  const u = usersData.find(user =>
    user.name.toLowerCase().includes(loginInput.value.toLowerCase())
  );
...
function modifyRecipes(e, category) {
  const r = recipeData.find(
    recipe => recipe.id === parseInt(e.target.getAttribute("id"))
  );
...
function displayReady() {
  ...
  document.querySelector(
    ".cook-status"
  ).innerHTML = `<p class="cook-message">You have all ingredients needed! Bon Appétit!</p>`;
}
